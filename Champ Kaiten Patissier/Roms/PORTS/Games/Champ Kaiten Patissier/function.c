...
int LoadGameFlag( char *fn )
{
	...
	snprintf(temp, sizeof(temp), "%s", fn);
	snprintf(temp2, sizeof(temp2), "%s%s%s", home_path, getenv("HOME"), temp);
#else
	char temp2[256];
	snprintf(temp2, sizeof(temp2), "%s", fn);
#endif
...
#ifdef HOME_PATH
	char temp[256], temp2[256];
	/* Use root directory to load maps instead of the home directory */
	if (strstr(fn, ".map"))
	{
		snprintf(temp2, sizeof(temp2), "%s", fn);
	}
...
//#ifdef GP2X
	...
	if(joys){
		...
		//if(SDL_JoystickGetButton(joys,  sdljbUp)) pad |= PAD_UP;
		//if(SDL_JoystickGetButton(joys,  sdljbLeft)) pad |= PAD_LEFT;
		//if(SDL_JoystickGetButton(joys,  sdljbDown)) pad |= PAD_DOWN;
		//if(SDL_JoystickGetButton(joys,  sdljbRight)) pad |= PAD_RIGHT;
